...
// Add performance-test data for Week 8.
// Fixed IDs make the seed operation safe to run more than once.
var performancePatients = new List<Patient>();
var performanceVitalSigns = new List<VitalSign>();
var performanceMedications = new List<Medication>();
var performanceAppointments = new List<Appointment>();

for (var i = 1; i <= 45; i++)
{
    var patientId = Guid.Parse(
        $"60000000-0000-0000-0000-{i:D12}");
    ...
    performancePatients.Add(new Patient
    {
        Id = patientId,
        FirstName = $"Performance{i:D2}",
        LastName = "TestPatient",
        DateOfBirth = new DateTime(1980, 1, 1).AddDays(i),
        Gender = i % 2 == 0 ? "Female" : "Male",
        Phone = $"555-02{i:D2}",
        Email = $"performance{i:D2}@example.com",
        Address = $"{i} Performance Street",
        City = "Performance City",
        State = "PT"
    });

    performanceVitalSigns.Add(new VitalSign
    {
        PatientId = patientId,
        RecordedAt = new DateTime(2026, 9, 1).AddDays(i),
        HeartRate = 70 + (i % 20),
        SystolicBloodPressure = 120 + (i % 10),
        DiastolicBloodPressure = 80 + (i % 8),
        OxygenSaturation = 97.0m,
        Temperature = 36.5m,
        RespiratoryRate = 16 + (i % 5)
    });

    performanceMedications.Add(new Medication
    {
        PatientId = patientId,
        Name = $"Performance Medication {i:D2}",
        Dosage = "10 mg",
        Frequency = "Once daily",
        StartDate = new DateTime(2026, 1, 1).AddDays(i)
    });

    performanceAppointments.Add(new Appointment
    {
        PatientId = patientId,
        AppointmentDate = new DateTime(2026, 10, 1).AddDays(i),
        Reason = "Performance test appointment",
        Status = "Scheduled",
        Notes = "Week 8 performance testing data"
    });
}

await context.Patients.AddRangeAsync(performancePatients);
await context.VitalSigns.AddRangeAsync(performanceVitalSigns);
await context.Medications.AddRangeAsync(performanceMedications);
await context.Appointments.AddRangeAsync(performanceAppointments);
